...
type Assignment = { office_id: string | null; department_id: string | null; team_id: string | null; designation_id: string | null; reporting_manager_id: string | null };
type Option = { id: string; name: string; officeId?: string; departmentId?: string };
type Options = { employees: (Option & { employeeCode: string; assignment: Assignment })[]; offices: Option[]; departments: Option[]; teams: Option[]; designations: Option[]; managers: Option[] };
...
const fields: [keyof Assignment, string, keyof Omit<Options, "employees">][] = [["office_id", "Office", "offices"], ["department_id", "Department", "departments"], ["team_id", "Team", "teams"], ["designation_id", "Designation", "designations"], ["reporting_manager_id", "Reporting manager", "managers"]];
const emptyAssignment: Assignment = { office_id: null, department_id: null, team_id: null, designation_id: null, reporting_manager_id: null };
  ...
  return <div ref={workspace} className="space-y-4">
    ...
    {mode && <DialogPanel title={decision ? labels[decision] : mode === "detail" ? "Transfer details" : mode === "edit" ? "Edit transfer draft" : mode === "supersede" ? "Supersede scheduled transfer" : can("Transfers.Create") ? "Prepare transfer" : "Recommend transfer"} onClose={close}>
      ...
      {decision ? <form onSubmit={decide} className="mt-4 space-y-3"><p className="text-sm">{decision === "approve" || decision === "apply" ? "Due transfers update organization assignments atomically. Previous assignments remain in history." : "This decision is recorded in immutable transfer history."}</p><Field label="Reason / comment *"><Textarea required maxLength={2000} value={comment} onChange={event => setComment(event.target.value)} /></Field><div className="flex flex-wrap justify-end gap-2"><Button type="button" variant="secondary" disabled={busy} onClick={() => setDecision(null)}>Back</Button><Button type="submit" disabled={busy}>{busy ? "Saving…" : "Confirm decision"}</Button></div></form> : mode !== "detail" ? <form onSubmit={save} className="mt-4 space-y-3">
        ...
        <div className="grid gap-3 sm:grid-cols-2">{fields.map(([key, label, collection]) => <Field key={key} label={`${label}${key === "designation_id" ? " *" : ""}`}><Select aria-label={`${label}${key === "designation_id" ? " *" : ""}`} required={key === "designation_id"} value={draft.proposed[key] ?? ""} disabled={(key === "department_id" && !draft.proposed.office_id) || (key === "team_id" && !draft.proposed.department_id)} onChange={event => { const proposed = { ...draft.proposed, [key]: event.target.value || null }; if (key === "office_id") { proposed.department_id = null; proposed.team_id = null; } if (key === "department_id") proposed.team_id = null; setDraft({ ...draft, proposed }); }}><option value="">{key === "designation_id" ? "Select designation" : "Not assigned"}</option>{options?.[collection].filter(o => key === "department_id" ? o.officeId === draft.proposed.office_id : key === "team_id" ? o.departmentId === draft.proposed.department_id : key === "reporting_manager_id" ? o.id !== draft.employee_id : true).map(o => <option key={o.id} value={o.id}>{o.name}</option>)}</Select></Field>)}</div>
      ...
      </form> : selected && <div className="mt-4 space-y-4"><div><p className="font-semibold">{selected.employee} · {selected.employeeCode}</p><p className="text-sm text-text-secondary">Requested by {selected.requester} · Effective {selected.effectiveDate}</p><StatusBadge value={selected.status} /></div>
