...
type Options = {
  banks: Named[];
  products: Named[];
  productVariants: Array<Named & { bankId: string; productId: string }>;
...
export default function FinancePage() {
  ...
  const ruleValidation = (() => {
    ...
    if (options?.products.find((item) => item.id === productId)?.code === "PF" && !productVariantId) return "Choose a PF product variant.";
    ...
    <section className="min-w-0 space-y-4">
      ...
      {!loading && currentTab === "payouts" ? (
        <div id="finance-panel-payouts" role="tabpanel" aria-labelledby="finance-tab-payouts" className="space-y-4">
          <ConfigurationWorkspace feedback={<><ErrorText>{error}</ErrorText>{message ? <p role="status" className="text-sm text-text-secondary">{message}</p> : null}</>} panelLabel="Payout period" controls={
          <Card className="p-3">
            <h2 className="mb-4 text-[length:var(--amafh-text-section)] font-medium">Payout period</h2>
          ...
          {components.length ? (
            <Card>
              <div className="mb-3 flex items-center justify-between gap-3"><h2 className="text-[length:var(--amafh-text-section)] font-semibold text-slate-900">Finance component drill-down</h2><Button type="button" variant="ghost" size="compact" onClick={() => setComponents([])}>Close</Button></div>
      ...
      {!loading && currentTab === "commission-rules" ? (
        <div id="finance-panel-commission-rules" role="tabpanel" aria-labelledby="finance-tab-commission-rules" className="space-y-4">
          <ListWorkspace feedback={<ErrorText>{error}</ErrorText>} title="Commission rule versions" filters={
          <Card className="p-3">
      ...
      {!loading && currentTab === "incentive-plans" ? (
        <div id="finance-panel-incentive-plans" role="tabpanel" aria-labelledby="finance-tab-incentive-plans" className="space-y-4">
          <ListWorkspace feedback={<ErrorText>{error}</ErrorText>} title="Incentive plan versions" filters={
          <Card className="p-3">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
          ...
          ) : <Card><EmptyState kind={planSearch ? "search" : "records"}>{planSearch ? "No records match the selected filters" : "No incentive plan versions exist yet."}</EmptyState></Card>}
