...
function TeamPerformance({ staff }: { staff: Staff[] }) {
  ...
  return <div data-testid="tl-member-targets"><ul className={styles.memberList}>{staff.map(person => {
    ...
    return <li key={person.id} data-testid={`tl-staff-row-${person.id}`} className={styles.memberRow}>
      <div className={styles.memberIdentity}><ProfilePhoto userId={person.id} fullName={person.name} size="list" className={styles.avatar} /><div><h3>{person.name}</h3><p className={styles.muted}>{missing ? "Target results unavailable" : mixed ? "Mixed target units · Average achievement" : `${target.measurement === "amount" ? "AED value" : "Application count"} · Average achievement`}</p></div>{!missing ? <Tooltip label={`Target basis for ${person.name}`} text="Achievement is the average of assigned target percentages at their configured milestones. Amount and count totals are kept separate. A full track is 100%; any overachievement is shown above it." align="right" /> : null}</div>
...
export function TlDashboard() {
  ...
  return <section className={cx(styles.dashboard, styles.reviewPreview)} data-testid="tl-dashboard" aria-busy={loading}>
    <span className="sr-only" data-testid="tl-last-update">Last update: {lastUpdatedAt ? <time dateTime={lastUpdatedAt}>{new Date(lastUpdatedAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit", second: "2-digit" })}</time> : "—"}</span>
    <div className={styles.workspaceBar} data-testid="tl-workspace-bar">
    ...
    {data ? <div role="tabpanel" id={`tl-panel-${tab}`} aria-labelledby={`tl-tab-${tab}`} className={styles.workspace} data-testid={tab === "review" ? "tl-review-workspace" : undefined}>
      {tab === "review" ? <>
        <div className={styles.reviewOverview}><div className={styles.reviewPriority}>
      ...
      {tab === "team" ? <div className={styles.teamWorkspace}><Disclosure title="Team targets & performance" testId="tl-team-performance" aside={<span className={styles.rowCount}>{data.staff.length} {data.staff.length === 1 ? "member" : "members"}</span>}><TeamPerformance staff={data.staff} /></Disclosure><aside><Disclosure title="My vs Team Applications" defaultOpen={false}><Distribution rows={data.charts.ownership} label="My versus team Applications" testId="tl-ownership-chart" /></Disclosure></aside></div> : null}
