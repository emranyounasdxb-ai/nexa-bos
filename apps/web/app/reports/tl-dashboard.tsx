...
import { IconArrowBackUp, IconArrowUpRight, IconBuildingBank, IconChartBar, IconChevronDown, IconChevronLeft, IconChevronRight, IconClock, IconFileDescription, IconInbox, IconRefresh, IconUsersGroup } from "@/components/icons";
...
function Metric({ label, value, prominent = false, history, trendKey }: { label: string; value: ReactNode; prominent?: boolean; history?: MetricHistory | null; trendKey?: string }) {
  return <div className={cx(styles.metric, prominent && styles.prominentMetric)}><dt>{label}</dt><dd>{value}</dd>{trendKey ? <dd className={styles.metricTrend}><TlSparkline history={history} label={label} metricKey={trendKey} /></dd> : null}</div>;
}
...
function TeamPerformance({ staff }: { staff: Staff[] }) {
  ...
  return <div data-testid="tl-member-targets"><ul className={styles.memberList}>{staff.map(person => {
    ...
    return <li key={person.id} data-testid={`tl-staff-row-${person.id}`} className={styles.memberRow}>
      ...
      <div className={styles.memberTarget}><dl className={styles.targetMetrics}><Metric label="Assigned" value={targetValue(target.assigned, target.measurement)} trendKey={`staff-${person.id}-assigned`} /><Metric label="Achieved" value={targetValue(target.achieved, target.measurement)} trendKey={`staff-${person.id}-achieved`} /><Metric label={target.remaining !== null && Number(target.remaining) < 0 ? "Exceeded by" : "Remaining"} value={targetValue(target.remaining === null ? null : String(Math.abs(Number(target.remaining))), target.measurement)} trendKey={`staff-${person.id}-remaining`} /></dl>{mixed ? <p className={styles.muted}>Totals are not combined across units.</p> : null}<div className={styles.memberProgress}>{missing ? null : <Achievement value={target.achievementPct} label={`${person.name} target achievement`} />}</div></div>
...
function PersonalPanels({ performance, attendance, period }: { performance: PersonalPerformance; attendance: PersonalAttendance; period: string }) {
  ...
  return <section aria-label="My performance and attendance" className={styles.personalGrid}>
    ...
    <Disclosure title="My Attendance" testId="my-attendance" aside={<Badge>{today.status}</Badge>}><p className={styles.chartNote}>Today · {today.date} · Read only</p><dl className={styles.dutyMetrics}><Metric prominent label="Duty" value={today.scheduledStart && today.scheduledEnd ? `${today.scheduledStart}–${today.scheduledEnd}` : "Not scheduled"} trendKey="attendance-duty" /><Metric prominent label="Check-in" value={today.actualCheckIn ?? "Not recorded"} trendKey="attendance-check-in" /><Metric prominent label="Check-out" value={today.actualCheckOut ?? "Not recorded"} trendKey="attendance-check-out" /><Metric prominent label="Worked" value={minutes(today.workedMinutes)} trendKey="attendance-worked" history={attendanceHistory("workedMinutes")} /></dl><dl className={styles.attendanceDetails}><Metric label="Late arrival" value={recorded ? minutes(today.lateMinutes) : "Not recorded"} trendKey="attendance-late" history={attendanceHistory("lateMinutes")} /><Metric label="Early departure" value={recorded ? minutes(today.earlyDepartureMinutes) : "Not recorded"} trendKey="attendance-early" history={attendanceHistory("earlyDepartureMinutes")} /><Metric label="Overtime" value={today.overtimeConfigured ? minutes(today.overtimeMinutes) : "Not configured"} trendKey="attendance-overtime" /></dl><div className={styles.monthSummary}><h3>{new Intl.DateTimeFormat("en", { month: "long", year: "numeric", timeZone: "UTC" }).format(new Date(`${attendance.month}T00:00:00Z`))}</h3><dl className={styles.personalStats}>{[["Present", attendance.summary.presentCount], ["Absent", attendance.summary.absentCount], ["Late", attendance.summary.lateCount], ["Leave", attendance.summary.leaveCount]].map(([label, value]) => <Metric key={label} label={String(label)} value={value} trendKey={`attendance-month-${String(label).toLowerCase()}`} />)}</dl></div><details className={styles.monthDetail}><summary className={focusRing}>Monthly attendance view ({attendance.items.length})</summary>{attendance.items.length ? <ul>{attendance.items.map(item => <li key={item.id}><div><time>{item.date}</time><Badge>{item.status}</Badge></div><dl className={styles.inlineMetrics}><Metric label="In / Out" value={`${item.checkIn ?? "—"} / ${item.checkOut ?? "—"}`} /><Metric label="Worked" value={minutes(item.workedMinutes)} /></dl></li>)}</ul> : <Empty>No attendance records are available this month.</Empty>}</details></Disclosure>
  </section>;
}
