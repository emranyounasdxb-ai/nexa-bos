...
export function AttendanceRecords({ embedded = false, mode = "daily" }: { embedded?: boolean; mode?: "daily" | "corrections" | "register" }) {
  const { can } = useAuth();
  ...
  const canManageRecords = mode === "daily" && canManageOffice;
  const canCorrect = mode !== "register" && can("Attendance.Correct");
  ...
  useEffect(() => {
    if (!can("Attendance.Daily")) return;
  ...
  useEffect(() => {
    if (!can("Attendance.Daily") || !filtersReady) return;
    void loadDay();
  ...
  return (
    <section className="space-y-4">
      ...
      />}
      <ListWorkspace feedback={<ErrorText>{error}</ErrorText>} title={!embedded ? "Daily register" : mode === "corrections" ? "Attendance corrections" : mode === "register" ? "Attendance register" : "Daily attendance"} filters={
