...
import { AttendanceExtendedReports, AttendancePolicyTools } from "../attendance-management";
...
export function AttendanceReportsView({ embedded = false }: { embedded?: boolean }) {
  const { can } = useAuth();
  ...
  return (
    <section className={`${styles.reportPage} space-y-4`}>
      ...
      {!embedded && <><AttendanceExtendedReports /><AttendancePolicyTools /></>}
    </section>
  );
}
