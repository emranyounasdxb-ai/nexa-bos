...
export function HrWorkflowSummary({ personal = false }: { personal?: boolean }) {
  ...
  return <Card className={styles.summary}><SectionHeader title={personal ? "My HR requests & contract" : "HR workflows"} actions={<Button variant="secondary" disabled={loading} onClick={() => void load()}>Refresh HR summary</Button>} />{error ? <ErrorText>{error}</ErrorText> : !data ? <LoadingState>Loading HR summary…</LoadingState> : own ? <div className={styles.personal}>
    {can("Leave.View") && <section className={styles.balances}><h3 className="text-lg font-semibold">My leave balances</h3>{own.balances?.length ? <ul className={styles.balanceGrid}>{own.balances.map((balance, index) => <li key={index}><strong>{balance.leaveType.name}</strong><dl><div><dt>Days available</dt><dd>{balance.available}</dd></div><div><dt>Pending</dt><dd>{balance.pending}</dd></div></dl></li>)}</ul> : <EmptyState>No configured leave balance.</EmptyState>}<details><summary className="cursor-pointer text-sm font-medium focus-visible:outline focus-visible:outline-2">My leave requests ({own.requests.length})</summary>{own.requests.length ? <ul className={styles.records}>{own.requests.map(row => <li key={row.id}><span>{row.startDate}–{row.endDate}</span><strong>{row.status}</strong></li>)}</ul> : <EmptyState>No leave requests.</EmptyState>}<Link className="text-sm text-brand-primary underline" href="/leave">Open my leave</Link></details></section>}
