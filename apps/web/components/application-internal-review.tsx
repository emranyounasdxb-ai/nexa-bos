...
export type InternalReview = {
  status: string; label: string; eventId: string | null; tlId: string | null; tlName?: string | null; reason: string | null;
...
export function ApplicationInternalReview({ applicationId, state, requestedAmount, onSaved, compact = false }: {
  compact?: boolean; applicationId: string; state: InternalReview; requestedAmount: string | null; onSaved: (message: string) => Promise<void>;
}) {
  ...
  async function confirm() {
    ...
    try {
      const path = action === "forward"
        ? `/api/v1/case-operations/applications/${applicationId}/book`
        : `/api/v1/applications/${applicationId}${action === "correct" ? "" : "/internal-review"}`;
      await apiRequest(path, getBrowserApiUrl(), {
        method: action === "correct" ? "PATCH" : "POST",
        body: JSON.stringify(action === "correct" ? { requested_amount: amount } : action === "forward" ? { expected_review_event_id: state.eventId } : { action, expected_event_id: state.eventId, reason: reason.trim() || null }),
      });
  ...
  return <section ref={panel} tabIndex={-1} data-testid="internal-review" className="min-w-0">
    <Card className="min-w-0 p-4">
      <div className="flex flex-wrap items-center justify-between gap-2"><h2 className="font-semibold">{compact ? "Case review" : "Internal Review tracker"}</h2>{!compact && <Badge>{state.label}</Badge>}</div>
      {!compact && <p className="mt-1 text-xs text-text-secondary">SE/TL → TL Booking → SM Approval → Coordinator → Bank Submission. The original Case Owner remains unchanged.</p>}
      {!compact && !state.tlId && state.status === "pending_review" && <p className="mt-2 text-sm text-danger">No valid TL assignment. Contact your administrator; COD processing is blocked.</p>}
      {state.reason && <p className="mt-2 break-words text-sm"><strong>Return reason:</strong> {state.reason}</p>}
      {!compact && <ol className="mt-3 grid gap-2 sm:grid-cols-3">{state.history.map(entry => <li key={entry.id} className="min-w-0 rounded-md border border-brand-border bg-surface p-2 text-xs"><p className="font-medium capitalize">{entry.action.replace("internal_", "").replaceAll("_", " ")}</p><time className="text-text-secondary">{new Date(entry.at).toLocaleString()}</time>{entry.reason && <p className="mt-1 break-words">{entry.reason}</p>}</li>)}</ol>}
      <div className="mt-3 flex flex-wrap gap-2">{state.actions.map(item => <Button key={item} disabled={busy} variant={item === "return" ? "secondary" : "primary"} onClick={event => open(item, event.currentTarget)}>{compact ? ({ forward: "Send for review", return: "Return for correction", resubmit: "Send to TL" })[item] : labels[item]}</Button>)}
    ...
    <dialog ref={dialog} aria-labelledby="review-confirm-title" aria-describedby="review-confirm-description" onCancel={event => { event.preventDefault(); close(); }} className="m-auto max-h-[calc(100dvh_-_32px)] w-[calc(100%_-_2rem)] max-w-md overflow-y-auto rounded-[24px] border border-brand-border bg-surface p-5 text-text-primary shadow-xl backdrop:bg-[#100916]/65 backdrop:backdrop-blur-sm">
      <form onSubmit={event => { event.preventDefault(); void confirm(); }}>
        <h2 id="review-confirm-title" className="text-lg font-semibold">{action && (compact ? ({ forward: "Send for review", return: "Return for correction", resubmit: "Send to TL", correct: "Correct requested amount" })[action] : labels[action])}</h2>
        <p id="review-confirm-description" className="mt-2 text-sm text-text-secondary">{compact ? "Confirm the case review action." : action === "return" ? "Return to the original SE for correction. A reason is required and recorded permanently." : action === "forward" ? "Lock the Case Owner and resolve the configured Office and Product Lane for processing." : action === "resubmit" ? "Send the corrected case back to the same TL for review." : "Correct the requested amount before resubmitting. This change is audited."}</p>
