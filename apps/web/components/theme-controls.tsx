...
function ThemeIcon({ theme }: { theme: Theme }) {
  return <svg aria-hidden="true" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
    {theme === "light" ? <><circle cx="12" cy="12" r="4" /><path d="M12 2v2m0 16v2M2 12h2m16 0h2M5 5l1.5 1.5m11 11L19 19M5 19l1.5-1.5m11-11L19 5" /></> : <path d="M20.5 14A8.5 8.5 0 0 1 10 3.5 8.5 8.5 0 1 0 20.5 14Z" />}
  </svg>;
}

export function ThemeControls({ className = "", compact = false }: { className?: string; compact?: boolean }) {
  const theme = useTheme();
  if (compact) {
    const next = theme === "light" ? "dark" : "light";
    return <div className={`amafh-theme-controls ${className}`} role="group" aria-label="Appearance">
      <button type="button" aria-label={`${next === "light" ? "Light" : "Dark"} theme`} title={`Switch to ${next} theme`} onClick={() => chooseTheme(next)}><ThemeIcon theme={next} /></button>
    </div>;
  }
  return (
