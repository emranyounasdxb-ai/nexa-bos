...
def _duration_label(seconds: object, days: object = None) -> str:
    if seconds is None:
        return (
            f"{days:g} calendar days (Date-only record)"
...
def _actor_label(actor: dict[str, Any] | None) -> str:
    ...
    return (
        " · ".join(
            str(actor[key]) for key in ("name", "code", "designation", "office") if actor.get(key)
        )
        or "Not recorded"
    )
    ...
    for entry in lifecycle["timeline"]:
        if entry["kind"] == "event" or entry.get("excludedFromTotals"):
            details.append(
                {
                    "Stage": entry["title"],
                    "Date": _readable_date(entry.get("from"), entry["precision"]),
                    "Custodian/Location": entry.get("employee")
                    or entry.get("location")
                    or "Not recorded",
                    "Office": entry.get("office") or "Not recorded",
                    "Condition": entry.get("condition") or "Not recorded",
                    "Reason/Note": " · ".join(
                        str(entry[key])
                        for key in ("reason", "note", "contextSource")
                        if entry.get(key)
                    )
                    or "Not recorded",
                    "Performed By": _actor_label(entry.get("actor")),
                    "Action Time": _readable_date((entry.get("actor") or {}).get("actionAt")),
                }
            )
            continue
        items.append(
            {
                ...
                "Stage": "Office/Location Transfer"
                if entry.get("actionCode") == "asset.office.transfer"
                else entry["title"],
                "Custodian Name": entry.get("employee") or "Not recorded",
                "Custodian Code": entry.get("employeeCode") or "Not recorded",
                "Location": entry.get("location") or "Not recorded",
                "Office": entry.get("office") or "Not recorded",
                "From": _readable_date(entry.get("from"), entry["precision"]),
                "To": "Current"
                if entry.get("active")
                else _readable_date(entry.get("to"), entry["precision"]),
                "Duration": _duration_label(
                    entry.get("durationSeconds"), entry.get("durationDays")
                ),
                "Status": entry.get("status") or "Not recorded",
                "Start Condition": entry.get("condition") or "Not recorded",
                "End Condition": entry.get("endCondition") or "Not recorded",
                "Reason/Note": " · ".join(
                    str(entry[key])
                    for key in ("reason", "note", "endReason", "endNote", "contextSource")
                    if entry.get(key)
                )
                or "Not recorded",
                "Opened By": _actor_label(entry.get("actor")),
                "Closed By": _actor_label(entry.get("endActor")),
            }
        )
        details.append(
            {
                "Stage": f"Period {len(items)} · {entry['title']}",
                "Start Condition": entry.get("condition") or "Not recorded",
                "End Condition": entry.get("endCondition") or "Not recorded",
                "Reason/Note": items[-1]["Reason/Note"],
                "Opened By": _actor_label(entry.get("actor")),
                "Opened At": _readable_date((entry.get("actor") or {}).get("actionAt")),
                "Closed By": _actor_label(entry.get("endActor")),
                "Closed At": _readable_date((entry.get("endActor") or {}).get("actionAt")),
            }
        )
    allocation = asset.get("currentAllocation") or {}
    summary = {
        "Registered": _readable_date(lifecycle["registeredAt"]),
        "Assignments": str(lifecycle["assignments"]),
    }
    for label, key in (
        ...
        total = lifecycle[key]
        has_recorded_period = any(
            e["kind"] == ("employee" if key == "assigned" else key)
            and e.get("durationSeconds") is not None
            for e in lifecycle["timeline"]
        )
        summary[label] = (
            "Not recorded"
            if not total["complete"] and not has_recorded_period
            else _duration_label(total["seconds"])
            + (" · recorded subtotal" if not total["complete"] else "")
        )
    if lifecycle.get("assignedDays"):
        summary["Assigned Time"] += f" + {lifecycle['assignedDays']} recorded calendar days"
    return {
        "title": "Asset Lifecycle History",
        ...
        "identity": {
            ...
            "Brand/Model": " / ".join(str(asset[k]) for k in ("brand", "model") if asset.get(k))
            or "Not recorded",
            "Serial/Identity": " / ".join(
                str(asset[k]) for k in ("serialNumber", "imei", "iccid") if asset.get(k)
            )
            or "Not recorded",
            "Current Status": asset["status"],
            "Condition": asset.get("condition") or "Not recorded",
            "Office": (asset.get("office") or {}).get("name") or "Not recorded",
            "Current Custodian": " · ".join(
                str(allocation[k]) for k in ("employeeName", "employeeCode") if allocation.get(k)
            )
            or next(
                (
                    str(e.get("location") or "Not recorded")
                    for e in lifecycle["timeline"]
                    if e.get("active") and not e.get("excludedFromTotals")
                ),
                "Not recorded",
            ),
        },
        ...
        "total": len(items),
        "periodActors": [
            (e.get("actor") or {}, e.get("endActor") or {})
            for e in lifecycle["timeline"]
            if e["kind"] != "event" and not e.get("excludedFromTotals")
        ],
        "exportPeriods": [
            e
            for e in lifecycle["timeline"]
        ...
        "registeredTimestamp": lifecycle["registeredAt"],
    }
...
def _lifecycle_report_rows(payload: dict[str, Any]) -> list[list[str]]:
    ...
    for index, item in enumerate(payload["items"], 1):
        custodian = (
            " · ".join(
                str(item[k])
                for k in ("Custodian Name", "Custodian Code")
                if item[k] != "Not recorded"
            )
            or item["Location"]
        )
        ...
        def actor_name(snapshot: dict[str, Any]) -> str:
            return (
                " · ".join(str(snapshot[k]) for k in ("name", "code") if snapshot.get(k))
                or "Not recorded"
            )

        performed = (
            "Opened: " + actor_name(opened) + ("; Closed: " + actor_name(closed) if closed else "")
        )
        rows.append(
            [
                str(index),
...
_LIFECYCLE_PAPER_TOKENS = {
    "amafh-table-header": "#3b3d73",
    "amafh-table-header-text": "#ffffff",
    "amafh-border": "#e1e6f0",
    "amafh-text": "#192340",
...
def _period_presentation(item: dict[str, Any]) -> list[tuple[str, str]]:
    custodian = (
        " · ".join(
            str(item[key])
            for key in ("Custodian Name", "Custodian Code")
            if item[key] != "Not recorded"
        )
        or item["Location"]
    )
    return [
...
class _LifecyclePDF(FPDF):
    def __init__(self, payload: dict[str, Any], generated_at: str) -> None:
        ...
        font_path = (
            Path(__file__).resolve().parents[3] / "web/public/fonts/manrope/manrope-variable.ttf"
        )
        if font_path.is_file():
            self.add_font(
                "Manrope", fname=str(font_path), variations={"": {"wght": 400}, "B": {"wght": 600}}
            )
    ...
    def header(self) -> None:
        ...
        self.set_font(self.report_font, "B", 16 * 0.75)
        self.cell(0, 6, "Asset Lifecycle History", new_x="LMARGIN", new_y="NEXT")
        self.set_font(self.report_font, size=8.5 * 0.75)
        for text in (
            f"{identity['Asset Code']} · {identity['Category']} · {identity['Brand/Model']}",
    ...
    def footer(self) -> None:
        ...
        self.set_font(self.report_font, size=7.5 * 0.75)
        self.cell(self.epw * 0.75, 5, f"Generated {self.generated_at}")
        self.cell(self.epw * 0.25, 5, f"Page {self.page_no()} of {{nb}}", align="R")
    ...
    def section_header(self, title: str, reserve: float = 18) -> None:
        ...
        self.set_font(self.report_font, "B", 10.5 * 0.75)
    ...
    def block_metrics(
        ...
        heights = [
            max(
                ...
                float(
                    self.multi_cell(
                        self.epw - 43, 3.2, self.report_text(value), dry_run=True, output="HEIGHT"
                    )
                ),
            )
            for _, value in fields
        ]
        self.set_font(self.report_font, "B", 8.5 * 0.75)
        heading_height = max(
            6,
            float(
                self.multi_cell(
                    self.epw - 8, 3.5, self.report_text(title), dry_run=True, output="HEIGHT"
                )
            )
            + 3,
        )
    ...
    def period_block(self, title: str, fields: list[tuple[str, str]]) -> None:
        ...
        if oversized or height > self.page_break_trigger - self.y:
            ...
            for label, value in fields:
                self.multi_cell(
                    0, 3.5, self.report_text(f"{label}: {value}"), new_x="LMARGIN", new_y="NEXT"
                )
            self.ln(2)
            return
        top = self.y
        ...
        for (label, value), row_height in zip(fields, heights, strict=True):
            self.set_xy(self.l_margin + 4, y)
            self.set_font(self.report_font, "B", 8 * 0.75)
            self.cell(label_width, 3.5, self.report_text(label))
            self.set_xy(self.l_margin + 4 + label_width, y)
            self.set_font(self.report_font, size=8.5 * 0.75)
            self.multi_cell(
                value_width, 3.2, self.report_text(value), new_x="LMARGIN", new_y="NEXT"
            )
            y += row_height
...
def _build_lifecycle_pdf(payload: dict[str, Any], actor: User) -> bytes:
    ...
    pdf.set_font(pdf.report_font, size=8.5 * 0.75)
    with pdf.table(
        ...
        headings_style=FontFace(
            emphasis="BOLD",
            color=_paper_rgb("amafh-table-header-text"),
            fill_color=_paper_rgb("amafh-table-header"),
        ),
    ) as table:
    ...
    pdf.multi_cell(
        ...
        3.5,
        pdf.report_text(
            f"Office: {payload['identity']['Office']} · "
            f"Condition: {payload['identity']['Condition']}"
        ),
        new_x="LMARGIN",
        new_y="NEXT",
    )
    pdf.multi_cell(
        ...
        3.5,
        pdf.report_text(
            f"Generated by: {actor.full_name} ({actor.user_code}) · "
            f"Scope: {payload['reportingScope']}"
        ),
        new_x="LMARGIN",
        new_y="NEXT",
    )
    ...
    if payload["items"]:
        ...
        first_period_height = pdf.block_metrics(
            f"1. {first['Stage']} · Status: {first['Status']}", _period_presentation(first)
        )[2]
    pdf.section_header(
        "Chronological lifecycle periods",
        reserve=min(first_period_height + 10, pdf.page_break_trigger - pdf.t_margin - 40),
    )
    for index, item in enumerate(payload["items"], 1):
        pdf.period_block(
            f"{index}. {item['Stage']} · Status: {item['Status']}", _period_presentation(item)
        )
    if not payload["items"]:
        ...
        pdf.multi_cell(
            0,
            4,
            "No recorded custody periods. Available legacy actions are listed below.",
            new_x="LMARGIN",
            new_y="NEXT",
        )
    ...
    if payload["details"]:
        ...
        first_detail_height = pdf.block_metrics(
            str(first_detail["Stage"]),
            [(label, str(value)) for label, value in first_detail.items() if label != "Stage"],
        )[2]
    pdf.section_header(
        "Secondary details · Individual actions and legacy records",
        reserve=min(first_detail_height + 10, pdf.page_break_trigger - pdf.t_margin - 40),
    )
    for item in payload["details"]:
        pdf.period_block(
            str(item["Stage"]),
            [(label, str(value)) for label, value in item.items() if label != "Stage"],
        )
    return bytes(pdf.output())
...
def _export_duration(seconds: object, days: object = None) -> str:
    ...
    if seconds is None:
        return (
            f"{days:g} calendar {'day' if days == 1 else 'days'}"
            if isinstance(days, (int, float))
            else "Not recorded"
        )
    minutes = int(max(0, float(seconds)) // 60)
...
def _export_summary(payload: dict[str, Any]) -> dict[str, str]:
    return {
        label: re.sub(
            r"(?<![0-9])1 recorded calendar days\b",
            "1 calendar day",
            re.sub(
                r"(?<![0-9])1 hours\b", "1 hour", re.sub(r"(?<![0-9])1 days\b", "1 day", str(value))
            ),
        )
        .replace("recorded subtotal", "partial total")
        .replace("recorded calendar days", "calendar days")
...
_LIFECYCLE_EXPORT_COLUMNS = [
    ...
    "Reason/Note",
    "Opened By Name",
    "Opened By Employee Code",
    "Opened By Designation",
    "Opened By Office",
    "Closed By Name",
    "Closed By Employee Code",
    "Closed By Designation",
    "Closed By Office",
]
        ...
        row.update(
            {
                ...
                "To": None if period.get("active") else period.get("to"),
                "Is Current": bool(period.get("active")),
                "Duration": _export_duration(seconds, days),
                "Duration Minutes": round(float(seconds) / 60, 6)
                if seconds is not None
                else float(days) * 1440
                if days is not None
                else None,
                "Status": item["Status"],
                "Start Condition": item["Start Condition"],
                "End Condition": item["End Condition"],
                "Reason/Note": " · ".join(
                    str(period[key])
                    for key in ("reason", "note", "endReason", "endNote")
                    if period.get(key)
                )
                or "Not recorded",
            }
...
def _build_lifecycle_excel(payload: dict[str, Any], actor: User) -> bytes:
    ...
    lifecycle.append(
        [key + " (UTC)" if key in ("From", "To") else key for key in _LIFECYCLE_EXPORT_COLUMNS]
    )
    for row in _export_period_rows(payload):
        lifecycle.append(
            [
                _excel_date(row[key]) if key in ("From", "To") else _safe_cell(row[key])
    ...
    for index, key in enumerate(_LIFECYCLE_EXPORT_COLUMNS, 1):
        lifecycle.column_dimensions[get_column_letter(index)].width = (
            ...
            if "Name" in key or "Designation" in key or key == "Stage"
            else 22
            if "Employee Code" in key or key == "Duration Minutes"
            else 18
        )
    for sheet in (summary, lifecycle):
        for row in sheet:
            for cell in row:
                ...
                cell.font = Font(
                    name="Manrope",
                    size=10,
                    color="FFFFFF" if heading else colors["amafh-text"],
                    bold=heading or (sheet == summary and cell.column == 1),
                )
                cell.fill = PatternFill(
                    "solid",
                    fgColor=colors["amafh-table-header"]
                    if heading
                    else colors["amafh-subtle"]
                    if cell.row % 2 == 0
                    else "FFFFFF",
                )
                cell.border = border
                ...
                if isinstance(cell.value, datetime):
                    date_only = (
                        sheet == lifecycle
                        and cell.row > 1
                        and cell.column in (7, 8)
                        and payload["exportPeriods"][cell.row - 2]["precision"] == "date"
                    )
            ...
            lines = max(
                (
                    sum(
                        max(
                            1,
                            (len(line) + int(sheet.column_dimensions[cell.column_letter].width) - 1)
                            // int(sheet.column_dimensions[cell.column_letter].width),
                        )
            ...
            sheet.row_dimensions[row[0].row].height = max(
                28 if row[0].row == 1 else 22, lines * 13 + 6
            )
    buffer = BytesIO()
    workbook.save(buffer)
    return buffer.getvalue()
...
def _build_lifecycle_print(payload: dict[str, Any], actor: User) -> str:
    ...
    generated_at = datetime.now(UTC).strftime("%d %b %Y, %I:%M %p UTC")
    summary = "".join(
        f"<div><dt>{safe(label)}</dt><dd>{safe(value)}</dd></div>"
        for label, value in _export_summary(payload).items()
    )
    periods = []
    for index, (item, period) in enumerate(
        zip(payload["items"], payload["exportPeriods"], strict=True), 1
    ):
        custodian = (
            " · ".join(
                str(item[key])
        ...
        fields = [
            ...
            (
                "Duration",
                _export_duration(period.get("durationSeconds"), period.get("durationDays")),
            ),
        ...
        facts = "".join(
            f"<div><dt>{safe(label)}</dt><dd>{safe(value)}</dd></div>" for label, value in fields
        )
        periods.append(
            f"<tr><td><article class='period'><h3>{index}. {safe(item['Stage'])} · "
            f"{safe(item['Status'])}</h3><dl>{facts}</dl></article></td></tr>"
        )
    tokens = ";".join(f"--{name}:{value}" for name, value in _LIFECYCLE_PAPER_TOKENS.items())
    return f"""<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">
    <title>Asset Lifecycle History</title><style>
    :root {{ {tokens}; }}
    ...
    @page {{ size:A4 portrait; margin:10mm 12mm 15mm;
      @bottom-left {{ content:'Generated {generated_at}';
        font:7.5px Manrope,Arial,sans-serif; color:var(--amafh-text-secondary); }}
      @bottom-right {{ content:'Page ' counter(page) ' of ' counter(pages);
        font:7.5px Manrope,Arial,sans-serif; }} }}
    body {{ font:8.5px Manrope,Arial,sans-serif; color:var(--amafh-text);
      margin:20px; letter-spacing:normal; }}
    header {{ break-inside:avoid; break-after:avoid; }}
    h1 {{ font-size:16px; margin:0 0 4px; }} p {{ margin:3px 0; }}
    .overview {{ break-inside:avoid; break-after:avoid; }}
    h3 {{ font-size:8.5px; margin:0; padding:4px 6px;
      background:var(--amafh-table-header); color:var(--amafh-table-header-text);
      break-after:avoid; }}
    dl {{ margin:0; display:grid; grid-template-columns:repeat(2,minmax(0,1fr)); }}
    dl > div {{ display:grid; grid-template-columns:80px minmax(0,1fr);
      padding:3px 6px; border-bottom:1px solid var(--amafh-border);
      break-inside:avoid; }}
    dt {{ font-weight:600; }} dd {{ margin:0; overflow-wrap:anywhere; }}
    .summary {{ grid-template-columns:repeat(5,minmax(0,1fr));
      border:1px solid var(--amafh-border); border-top:3px solid var(--amafh-table-header);
      border-radius:6px; background:var(--amafh-subtle); margin:6px 0; }}
    .summary > div {{ display:block; padding:5px;
      border-right:1px solid var(--amafh-border); border-bottom:0; }}
    .summary > div:last-child {{ border-right:0; }} .summary dd {{ margin-top:3px; }}
    .periods {{ width:100%; border-collapse:separate; border-spacing:0 4px; }}
    .periods th {{ background:var(--amafh-table-header);
      color:var(--amafh-table-header-text); font-size:9px;
      text-align:left; padding:5px; }} .periods td {{ padding:0; }}
    thead {{ display:table-header-group; }}
    tr {{ break-inside:avoid; page-break-inside:avoid; }}
    .period {{ border:1px solid var(--amafh-border); border-radius:6px;
      overflow:hidden; break-inside:avoid; }}
    @media print {{ button,details,summary,nav {{ display:none!important; }}
      body {{ margin:0; }} .period {{ overflow:visible; }} }}
    </style></head><body><header><h1>Asset Lifecycle History</h1>
    <p>{safe(identity["Asset Code"])} · {safe(identity["Category"])} ·
    {safe(identity["Brand/Model"])}</p>
    <p>Identity: {safe(identity["Serial/Identity"])}</p>
    <p>{safe(identity["Current Status"])} · {safe(identity["Current Custodian"])}</p>
    </header><section class="overview"><dl class="summary">{summary}</dl>
    <p>Office: {safe(identity["Office"])} · Condition: {safe(identity["Condition"])}</p>
    <p>Prepared by: {safe(actor.full_name)} ({safe(actor.user_code)})</p></section>
    <table class="periods"><thead><tr><th>Chronological lifecycle periods ·
    {safe(identity["Asset Code"])}</th></tr></thead><tbody>
    {"".join(periods) or "<tr><td>No recorded custody periods.</td></tr>"}
...
def build_lifecycle_csv(payload: dict[str, Any]) -> bytes:
    ...
    writer.writerow(
        [f"{key} (UTC)" if key in ("From", "To") else key for key in _LIFECYCLE_EXPORT_COLUMNS]
    )
    for row in _export_period_rows(payload):
        values = []
        for key in _LIFECYCLE_EXPORT_COLUMNS:
