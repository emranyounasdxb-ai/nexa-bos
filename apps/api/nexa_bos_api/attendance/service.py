...
from nexa_bos_api.identity.access import can_view_user
...
async def _assert_employee_visible(session: AsyncSession, actor: User, employee: User) -> None:
    ...
    if (allowed is not None and employee.id not in allowed) or not await can_view_user(
...
async def apply_calculation(
    ...
    *,
    ...
    result = calculate_attendance(
        ...
        or await _office_holiday(session, employee.office_id, record.attendance_date),
    )
    ...
    *,
    ...
    for entry in sorted(entries, key=lambda value: str(value.employee_id)):
        if attendance_date > business_today():
            raise AppError(
                status_code=422,
                code="ATTENDANCE_FUTURE_ENTRY",
                message=(
                    "Attendance cannot be recorded for a future date. "
                    "Record already approved leave through Leave & Holidays instead."
                ),
            )
        ...
        if existing:
            same = (
                ...
                and existing.time_in == entry.time_in
                and existing.time_out == entry.time_out
                and (existing.notes or "") == (entry.notes or "").strip()
                and existing.leave_type_id == entry.leave_type_id
            )
            if not same:
                raise AppError(
                    status_code=409,
                    ...
                    message=(
                        "Attendance already exists. Use Correct with a mandatory reason; "
                        "existing attendance cannot be overwritten."
                    ),
                )
...
async def correct_attendance(
    ...
    origin = (
        await session.get(AttendanceProvenance, row.id) if await storage_ready(session) else None
    )
    ...
    if payload.expected_updated_at is not None and row.updated_at != payload.expected_updated_at:
        raise AppError(
            ...
            message="Attendance changed since this view was opened. Refresh before correcting it.",
        )
...
async def _office_holiday(session: AsyncSession, office_id: UUID | None, on_date: date) -> bool:
    ...
    return bool(
        await session.scalar(
            select(AttendanceOfficeHoliday.id)
            .where(
                AttendanceOfficeHoliday.office_id == office_id,
                AttendanceOfficeHoliday.holiday_date == on_date,
            )
            .limit(1)
        )
    )
