...
for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
  test(`approval centre independent decisions and own summary at ${viewport.width}`, async ({ page, request }, info) => {
    ...
    await selectBrandedOption(page.getByLabel("Module", { exact: true }), "Contracts"); await expect(page.getByText("No records match the selected filters")).toBeVisible(); await captureViewportThemes(page, info.outputPath(`approval-empty-${viewport.width}.png`), page.getByText("No records match the selected filters", { exact: true })); await page.goBack(); await expect(row).toBeVisible();
