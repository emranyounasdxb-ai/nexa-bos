...
for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
  test(`approval centre independent decisions and own summary at ${viewport.width}`, async ({ page, request }, info) => {
    ...
    await page.screenshot({ path: info.outputPath(`approval-queue-${viewport.width}.png`), fullPage: false });
    ...
    await selectBrandedOption(page.getByLabel("Module", { exact: true }), "Contracts"); await expect(page.getByText("No requests match these filters.")).toBeVisible(); await page.screenshot({ path: info.outputPath(`approval-empty-${viewport.width}.png`), fullPage: false }); await page.goBack(); await expect(row).toBeVisible();
    ...
    if (viewport.width < 1024) await page.getByRole("button", { name: "Open navigation" }).click(); await page.getByRole("button", { name: "Open user menu" }).focus(); await expect(page.getByRole("button", { name: "Open user menu" })).toBeFocused(); await page.keyboard.press("Enter"); await expect(page.getByRole("menuitem", { name: "Sign out" })).toBeVisible(); await page.getByRole("menuitem", { name: "Sign out" }).click(); await expect(page).toHaveURL(/\/login/);
