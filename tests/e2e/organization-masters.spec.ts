...
test("Organization masters use URL tabs, filters, dependent drawers, and accessible dismissal", async ({
  ...
  await expect(drawer.getByRole("textbox", { name: /Immutable code/ })).toHaveCount(0);
  await drawer.getByRole("textbox", { name: /Office name/ }).fill("Automatically coded office");
  const officeCreate = page.waitForResponse((response) => response.url().endsWith("/api/v1/offices") && response.request().method() === "POST");
  await drawer.getByRole("button", { name: "Save" }).click();
  expect((await officeCreate).status()).toBe(200);
  await expect(drawer).toHaveCount(0);
...
for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
  test(`Business Unit creation and history-preserving OWNER deletion ${viewport.width}`, async ({ page, request }, testInfo) => {
    ...
    await expect(drawer.getByRole("textbox", { name: /Immutable code/ })).toHaveCount(0);
    const createdUnit = page.waitForResponse((response) => response.url().endsWith("/api/v1/business-units") && response.request().method() === "POST");
    await drawer.getByRole("button", { name: "Save", exact: true }).click();
    const code = ((await (await createdUnit).json()) as { code: string }).code;
    await expect(drawer).toHaveCount(0);
    ...
    await expect(teamDrawer.getByRole("textbox", { name: /Immutable code/ })).toHaveCount(0);
    ...
    const createdTeamBody = (await teamResponse.json()) as { businessUnitId: string; code: string };
    expect(createdTeamBody.businessUnitId).toBe(seeded.teamA.businessUnitId);
    await expect(teamDrawer).toHaveCount(0);
    await page.getByLabel("Search teams").fill(createdTeamBody.code);
    await expect(region).toContainText("Explicit Business Unit team");
    await expect(region).not.toContainText(createdTeamBody.code);
    await expectNoPageOverflow(page);
  });
}
