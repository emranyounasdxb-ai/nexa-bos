...
test("elapsed TAT, mark delay, list badge, timeline, and correction", async ({ page, request }) => {
  ...
  await expect(page.getByText("Delay recorded", { exact: true })).toBeVisible();
  await page.getByRole("tab", { name: "Workflow & TAT" }).click();
  const delayState = page.getByRole("heading", { name: "Delay state" }).locator("..");
  await expect(delayState.getByText("Bank", { exact: true })).toBeVisible();
  ...
  await expect(page.getByText("Delay correction recorded", { exact: true })).toBeVisible();
