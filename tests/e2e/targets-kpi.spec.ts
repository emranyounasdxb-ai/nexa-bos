...
test("owner targets, KPI scorecards, profile section, and scoped isolation", async ({
  ...
  await page.getByLabel("Weight 1").fill("40");
  ...
  await expect(page.getByText("Activation failed").or(page.getByText("must have weights totaling exactly 100%"))).toBeVisible();
